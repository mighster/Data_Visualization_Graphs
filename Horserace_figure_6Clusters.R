...
#Import new df
df <-read.csv("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/NormalizedBLUPsAllDays_292.csv")
...
Day6NormalizedOutput <- data.frame(matrix(vector(),294,1, dimnames=list(c(), c("Entry"))))
...
#Create empty data frame for BLUP output
Day9NormalizedOutput <- data.frame(matrix(vector(),294,1, dimnames=list(c(), c("Entry"))))
...
write.csv(Entry, "C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/2017 Data - Growth Chamber/Data4Paper/entry.csv")
...
#Create empty data frame for BLUP output
Day12NormalizedOutput <- data.frame(matrix(vector(),294,1, dimnames=list(c(), c("Entry"))))
...
#########################################################################################################################
day_list <- list(Day6_data, Day9_data, Day12_data)
column_names <- colnames(Day6_data[,c(2:45)])
str(Day6_data)
...
colnames(DayDataOutput) <- colnames(Day6_data[,c(2:45)])
colnames(DayDataOutput)
NewData <- DayDataOutput[,c(1:34,43:44)]
...
tiff("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/HorseRace_Day6.tiff", units = "in" ,height = 8, width = 21, res=200)
  ...
  geom_line(data=Day6_clust, aes(x=Day6_clust$variable,y=Day6_clust$value, group=Cluster, color=Cluster), size = 1.5) +
...
dev.off()
  ...
  tiff("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/HorseRace_Day9.tiff", units = "in" ,height = 8, width = 21, res=200)
  ggplot(data=Day9_melt, aes(x=Day9_melt$variable, y=Day9_melt$value)) +
    geom_point(size =2,na.rm=TRUE) +
    ylab("Normalized BLUP Value") + xlab("RSA Trait at 9d") + 
    scale_y_continuous(limits = c(0,1)) +
    geom_line(data=Day9_clust, aes(x=Day9_clust$variable,y=Day9_clust$value, group=Cluster, color=Cluster), size = 1.5) +
    scale_color_manual(values=c("yellow", "deeppink", "darkorchid","green","blue", "red")) +
    theme_bw() +
    theme(text = element_text(size=15),axis.text.x = element_text(angle = 45, hjust=1), panel.background = element_blank(), panel.grid = element_blank(), strip.background = element_blank())
  dev.off()
  
  tiff("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/HorseRace_Day12.tiff", units = "in" ,height = 8, width = 21, res=200)
  ggplot(data=Day12_melt, aes(x=Day12_melt$variable, y=Day12_melt$value)) +
    geom_point(size =2,na.rm=TRUE) +
    ylab("Normalized BLUP Value") + xlab("RSA Trait at 9d") + 
    scale_y_continuous(limits = c(0,1)) +
    geom_line(data=Day12_clust, aes(x=Day12_clust$variable,y=Day12_clust$value, group=Cluster, color=Cluster), size = 1.5) +
    scale_color_manual(values=c("yellow", "deeppink", "darkorchid","green","blue", "red")) +
    theme_bw() +
    theme(text = element_text(size=15),axis.text.x = element_text(angle = 45, hjust=1), panel.background = element_blank(), panel.grid = element_blank(), strip.background = element_blank())
  dev.off()
