...
day6 <-  Day6NormalizedOutput %>%
  dplyr::select(Country,TRL,PRL,WID,CVA,LRB,VOL,DIA,LRA,BSH,SOL2,LED,RHZO,TRLUpper,TRLLower,WDR,SRL.LRB,Root_weight,Shoot_weight)  #select RSA traits to use for particular ideotype
Day6_melt <-  melt(day6, id.vars = "Country")

day9 <-  Day9NormalizedOutput %>%
  select(Country,TRL,PRL,WID,CVA,LRB,VOL,DIA,LRA,BSH,SOL2,LED,RHZO,TRLUpper,TRLLower,WDR,SRL.LRB,Root_weight,Shoot_weight) #select RSA traits to use for particular ideotype
Day9_melt <-  melt(day9, id.vars = "Country")

day12 <-  Day12NormalizedOutput %>%
  select(Country,TRL,PRL,WID,CVA,LRB,VOL,DIA,LRA,BSH,SOL2,LED,RHZO,TRLUpper,TRLLower,WDR,SRL.LRB,Root_weight,Shoot_weight) #select RSA traits to use for particular ideotypedeotype
...
SelectedTraits <-  NewData %>%
                  select(Day,Country,TRL,PRL,WID,CVA,VOL,DIA,LRB,LRA,BSH,SOL2,LED,RHZO,TRLUpper,TRLLower,WDR,SRL.LRB,Root_weight,Shoot_weight) #select RSA traits to use for particular ideotype
...
tiff("C:/Users/falk/Google Drive/PhD/Papers/RSA ARIA Methods Paper - Zaki Vahid Kevin/Figures/Horserace/HorseRace_MG2_Country_Day6_Feb21.tiff", units = "in" ,height = 5, width = 8, res=200)
ggplot(data=Day6_melt, aes(x=Day6_melt$variable, y=Day6_melt$value)) +
  ...
  geom_line(data=Day6_clust, aes(x=Day6_clust$variable,y=Day6_clust$value, group=Country, color=Country), size = 1.5) +
  scale_color_manual(values=c("blue","red","green3","orange","magenta","black"))+
  labs(color=("Country")) +
  theme_bw() +
  theme(text = element_text(size=15),axis.text.x = element_text(angle = 90, hjust=1), panel.background = element_blank(), panel.grid = element_blank(), strip.background = element_blank(), legend.text.align = 1)
 dev.off()

 tiff("C:/Users/falk/Google Drive/PhD/Papers/RSA ARIA Methods Paper - Zaki Vahid Kevin/Figures/Horserace/HorseRace_MG2_Country_Day9_Feb21.tiff", units = "in" ,height = 5, width = 8, res=200)
 ggplot(data=Day9_melt, aes(x=Day9_melt$variable, y=Day9_melt$value)) +
   ...
   scale_y_continuous(limits = c(-4,4)) +
   geom_line(data=Day9_clust, aes(x=Day9_clust$variable,y=Day9_clust$value, group=Country, color=Country), size = 1.5) +
   scale_color_manual(values=c("blue","red","green3","orange","magenta","black"))+
   labs(color=("Country")) +
   theme_bw() +
   theme(text = element_text(size=15),axis.text.x = element_text(angle = 90, hjust=1), panel.background = element_blank(), panel.grid = element_blank(), strip.background = element_blank(), legend.text.align = 1)
 ...
 tiff("C:/Users/falk/Google Drive/PhD/Papers/RSA ARIA Methods Paper - Zaki Vahid Kevin/Figures/Horserace/HorseRace_MG2_Country_Day12_Feb21.tiff", units = "in" ,height = 5, width = 8, res=200)
 ggplot(data=Day12_melt, aes(x=Day12_melt$variable, y=Day12_melt$value)) +
   ...
   scale_y_continuous(limits = c(-4,4)) +
   geom_line(data=Day12_clust, aes(x=Day12_clust$variable,y=Day12_clust$value, group=Country, color=Country), size = 1.5) +
   scale_color_manual(values=c("blue","red","green3","orange","magenta","black"))+
   labs(color=("Country")) +
   theme_bw() +
   theme(text = element_text(size=15),axis.text.x = element_text(angle = 90, hjust=1), panel.background = element_blank(), panel.grid = element_blank(), strip.background = element_blank(), legend.text.align = 1)
