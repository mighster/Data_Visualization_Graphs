...
library(tidyverse)
library(colorspace)
install.packages("stat-ellipse")
library(stat-ellipse)
...
Best <- as.numeric(metadata$BestGenotype)
str(Best)
...
tiff("C:/Users/falk/Google Drive/PhD/PhD Projects/Blue Steel/Paper#2/Genetic Distance/PCA_Maximum_March19.tiff",compression = "lzw", width = 6, height = 5, units = 'in', res = 500)
ggplot(pca1_loading, aes(x = PC1, y = PC2, color = Best)) +
  geom_point(alpha=0.99,size=2.75) +
  labs(x = c(pca1_loading$sdev[1],"PC1 (11.3 %)"), y = c(pca1_loading$sdev[2], "PC2 (6.2 %)"), color="Best")+
  labs(color=expression(atop('Best Overall', 'Rank (n = 292)'))) +
  theme_classic()+
  labs(color=expression(atop('Maximum', 'iRoot Rank'))) +
  scale_color_gradient2(midpoint=135, low="blue", mid="white",
                        high="red", space ="Lab" )+
